function [x,y,kf,ko]=sdp_linp(A,B,C,x,y)
% function [x,y,kf,ko]=sdp_linp(A,B,C,x,y)
%
% solves linear programs B+A*x>0, C'*x->min, y>0, A'*y=C, B'*y->min
% with initial guesses 
%    x=x0 (default x0=-(A'*A)\(A'*B)) 
%    y=y0 (default y0=max(0.1,A'*((A'*A)\C)))
...
for kf=1:N,                    % feasibility loop
    if ~all(y>0), 
        disp(rd)
        error([num2str(kf) '[f]: min(y)=' num2str(min(y))]); 
    end
    Ax=A*x;
    w=B+Ax;
    ...
    v1=w.*Dy(:,2)+y.*ADx(:,2);
    v2=Dy(:,2).*ADx(:,2);
    pp=[2*(v2'*v2) -3*(v1'*v2) 2*(v0'*v2)+v1'*v1 -v0'*v1];
    rd=roots(pp);
    rd=rd(imag(rd)==0);
    %sc=min([min(v(v1>0)./v1(v1>0)) min(rd(rd>0))]);
