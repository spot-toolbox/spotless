...
% function [x,p,M]=decomp(q)
%
% If q=const then x is empty 0-by-1 mtpoly, p=zeros(0,1), M=double(p(:)).
% Otherwise x is k-by-1 free mtpoly, p is a sparse N-by-k nonnegative 
...
nx=length(idx0);
x=mtpoly(nx,1,[(1:nx)' ones(nx,1) idx0 ones(nx,2)]);
