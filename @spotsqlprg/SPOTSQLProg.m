    ...
    methods
       ...
       function sol = optimizeStandardDual(pr,objective,options)
            ...
            mAT = sparse(varNo(i),j,s,KvarCnt,size(mAT,2)) + ...
                  sparse(varNoSymm(i),j,s,KvarCnt,size(mAT,2));
            mAT(varNoDiag) = mAT(varNoDiag)/2;
            
            
            [i,j,s] = find(-cm);
            c = sparse(varNo(i),j,s,KvarCnt,1) + sparse(varNoSymm(i),j,s,KvarCnt,1);
            c(varNoDiag) = c(varNoDiag)/2;
            
            % Problem.  We've double counted the diagonal entries.
            
            
            [x,y,info] = sedumi(-mAT.',b,c,K,options);
